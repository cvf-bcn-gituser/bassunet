...
class BassUNet:
    ...
    def evaluate_accuracy(gt_measures, measures, matching_window_size):
        def match_events(gt_measures, measures, matching_window_size):

            # In case of performance issues for big piecs,
            # we could try to use simpler/faster local algorithm.
            m = scipy.spatial.distance_matrix([[x] for x in gt_measures], [[x] for x in measures])
            # don't consider events which are out of matching window size
            big_distance = 10 ** 6
            m[m > matching_window_size] = big_distance
            ...
            for (x, y) in zip(row_ind, col_ind):
                if abs(gt_measures[x] - measures[y]) <= matching_window_size:
                    result.append((x, y))    
            return result
        matching = match_events(gt_measures,measures,matching_window_size)
        precision = float(len(matching)) / len(measures)
        recall = float(len(matching)) / len(gt_measures)
    ...
    def scrutinize_accuracy(gt_onsets, onsets, gt_offsets, offsets, matching_window_size,scale_factor):
        def match_events(gt_onsets, onsets, gt_offsets, offsets, matching_window_size,scale_factor):
            ...
            for (xn, yn) in zip(row_ons, col_ons):
                if abs(onsets[yn] - gt_onsets[xn]) <= matching_window_size:
                    ...
                    if offsets[yn-1]>=onsets[yn]:
                        offsets[yn-1]=onsets[yn]
                    if abs(offsets[yn] - gt_offsets[xn]) <= matching_window_size*scale_factor:
                        result.append((xn, yn))    
            return result
                    
        matching = match_events(gt_onsets, onsets, gt_offsets, offsets, matching_window_size,scale_factor)
...
if __name__ == '__main__':
    ...
    # TODO
	# REfactor this to import onl the CSV files
	
	# Simplify Onsets to just consider the onsets. No "stealing .
	# EVL measure based pruely on onsets.
	# Cosnider two WAV files for inspection 002 and 004 maybe 10 and 12.
	# Check if PRF values are in line with the histogra,s
	
	# Put a version in the forked abesser area. Communicate work done , but no Pull request.
	# This will be something good to report in presentation

    the_file= "PRF_measure.csv"
    f1 = open(the_file, "w")
    f1.write(" index, precision, recall, f_measure_value \n")
	
    the_missing_data_file= "MissingOutedStolenOnsets.csv"
    fm = open(the_missing_data_file, "w")
    fm.write(" index, missing onset, outed offsets, stolen onsets \n")
    ...
    while index <= 17:
        ...
        precision_on, recall_on, f_measure_value_on=BassUNet.evaluate_accuracy(ground_t_onsets_array[index-1], onset, matching_window_size)
        print(index,"PRFs", precision_on, recall_on, f_measure_value_on)
        #precision_on, recall_on, f_measure_value_on= BassUNet.scrutinize_accuracy(ground_t_onsets_array[index-1], onset, ground_t_offsets_array[index-1], offset,matching_window_size,1.0)
        
        total_p_on += precision_on
        total_r_on += recall_on
        total_f_on += f_measure_value_on
        print(index ,"ONSET precision, recall, f_measure_value \n",precision_on, recall_on, f_measure_value_on)
        print("\n")
        p_on.append(precision_on)
        r_on.append(recall_on)
        f_on.append(f_measure_value_on)
        
        string_to_write_on =str(index) + "," + str(precision_on) + "," + str(recall_on) + "," + str(f_measure_value_on)+"\n"
        f1.write(string_to_write_on)

        p1=ground_t_onsets_array[index-1]
        p2=onset
        p3=ground_t_durations_array[index-1] 
        p4=duration
        p5=matching_window_size
        onset_deviations, offset_deviations, missing_onset_notes,missing_offset_notes,stolen_onset=   BassUNet.match_onset1(p1,p2,p3,p4,p5)    
       
        missing_string = str(index) + "," + str(missing_onset_notes) + " ," + str(missing_offset_notes) +  ","+str(stolen_onset) +"\n"
        print(index,missing_string)
        fm.write(missing_string)
        ...
        j=0 
        fn_offset_dev = fn_wav.replace('.wav', 'offset_dev')         
        the_file= fn_offset_dev+'.csv'
        f3 = open(the_file, "w")    
        
        while j < len(fn_offset_dev):
            f3.write(str(fn_offset_dev[j]))
            f3.write("\n")
            j+=1
        #######################################################################################################################
        ...
        print(fn_csv, ' saved!')
        index+=1
    print("Total of file processed:", index)
    divisor= index-1.0
    print("PRF", round(total_p_on/(divisor),2), round(total_r_on/(divisor),2),round(total_f_on/(divisor),2))