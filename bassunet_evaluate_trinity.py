...
if __name__ == '__main__':
    ...
    index= 1
    while index <= 8:
       student_file_audio_array.append(student_head+str(index)+student_tail)
       index+=1

    print(student_file_audio_array)
    ...
    of = open("bj_of_gt.csv", "r")
    of_array = []
    for x in of:
       of_array.append(float(x))
    print(len(of_array))
    on = open("bj_on_gt.csv", "r")
    on_array = []
    for x in on:
    ...
    ground_t_durations = array(of_array)-array(on_array)
    ground_t_durations_array.append(array(ground_t_durations))
		
	###   Do the reference
	
    the_prf_file= "PRF_BJ_TRINITY_REFERENCE_MEASURE.txt"
...
1
    ...
    bun = BassUNet()
    # transcription
    t, f0, onset, duration, pitch = bun.run(filename)
    ...
    precision, recal, f_measure_value=BassUNet.evaluate_accuracy(ground_t_onsets_array[0], onset, matching_window_size)
    total_p += precision
    total_r += recal
    total_f += f_measure_value
    print(" precision, recal, f_measure_value \n",precision, recal, f_measure_value)
    print("\n")
    p.append(precision)
    r.append(recal)
    f.append(f_measure_value)
    index= 0
    string_to_write="0" + "\t" + str(precision) + "\t" + str(recal) + "\t" + str(f_measure_value)+"\n"
    f1.write(string_to_write)
    p1=ground_t_onsets_array[0]
    p2=onset
    p3=ground_t_durations_array[0] 
    p4=duration
    p5=matching_window_size
    onset_deviations, duration_deviations, missing_onset_notes =   BassUNet.match_duration(p1,p2,p3,p4,p5)
    print(" missing_onset_notes",missing_onset_notes)
    missing_string = str(index) + "missing_onset_notes " + str(missing_onset_notes)
    f1.write(missing_string)
    f1.write("\n")
    j=0 
	
    the_file="bj_reference_onset_dev"+str(index)+'.csv'
    f2 = open(the_file, "w")
    ...
    j=0 
    the_file= "bj_reference_duration_dev"+str(index)+'.csv'
    f3 = open(the_file, "w")    
    ...
    # export pitch track
    #n_csv = fn_wav.replace('.wav', '_trinity_bass_f0.csv')
    fn_csv = 'trinity_bj_bass_reference_f0.csv'
    BassUNet.export_pitch_track_as_csv(t, f0, fn_csv)
    print(fn_csv, ' saved!')

    # export notes
    #fn_csv = fn_wav.replace('.wav', '_trinity_bass_notes.csv')
    fn_csv = 'trinity_bj_bass_reference_notes.csv'
    BassUNet.export_notes_as_csv(onset, duration, pitch, fn_csv)
    print(fn_csv, ' saved!')

	###
    the_prf_file= "tcl_output\\PRF_BJ_TRINITY_measure.csv"
    f1 = open(the_prf_file, "w")
    f1.write(" precision, recall, f_measure_value \n")
	
    index=1

    # Loop though 8 students of Billy Jean
    while index <= 8:


	    bun = BassUNet()
	    # transcription
	    t, f0, onset, duration, pitch = bun.run(student_file_audio_array[index-1])
	    ...
	    total_r += recal
	    total_f += f_measure_value
	    print(index ," precision, recal, f_measure_value \n",precision, recal, f_measure_value)
	    print("\n")
	    p.append(precision)
	    r.append(recal)
	    f.append(f_measure_value)

	    string_to_write=str(index) + "," + str(round(precision,3)) + "," + str(round(recal,3)) + "," + str(round(f_measure_value,3))+"\n"
	    f1.write(string_to_write)
	    p1=ground_t_onsets_array[0]
	    p2=onset
	    p3=ground_t_durations_array[0] 
	    ...
	    p5=matching_window_size
	    onset_deviations, duration_deviations, missing_onset_notes =   BassUNet.match_duration(p1,p2,p3,p4,p5)
	    print(index," missing_onset_notes",missing_onset_notes)
	    missing_string = str(index) + "missing_onset_notes " + str(missing_onset_notes)
	 
	    j=0 
	    
	    the_file="tcl_output\\bj_onset_dev"+str(index)+'.csv'
	    f2 = open(the_file, "w")
	    while j < len(onset_deviations):
	        f2.write(str(onset_deviations[j]))
	        f2.write("\n")
	    ...
	    j=0 
	    the_file= "tcl_output\\bj_duration_dev"+str(index)+'.csv'
	    f3 = open(the_file, "w")    
	    ...
	    # export pitch track
	    #n_csv = fn_wav.replace('.wav', '_trinity_bass_f0.csv')
	    fn_csv = 'trinity_bass_f0.csv'
	    BassUNet.export_pitch_track_as_csv(t, f0, fn_csv)
	    print(fn_csv, ' saved!')

	    # export notes
	    #fn_csv = fn_wav.replace('.wav', '_trinity_bass_notes.csv')
	    fn_csv = 'trinity_bass_notes.csv'
	    BassUNet.export_notes_as_csv(onset, duration, pitch, fn_csv)
	    print(fn_csv, ' saved!')
	    index+=1
